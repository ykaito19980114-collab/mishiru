...
function fallbackStep2(input: QuestionFreeInput, selectedRq: RQCandidate, step1: Step1Response): Step2Response {
  ...
  const target = step1.decomposition.target || selectedRq.rq_title;
  const phenomenon = step1.decomposition.phenomenon || selectedRq.what_we_learn;
  const context = step1.decomposition.context || "入力された具体的な場面";
  ...
  const literatureReview = isEffectStudy ? {
    knowns: [
      `${target}に近い対象では、接合前の表面処理や加工条件が界面の形状・濡れ性・接着状態を変え、初期強度に影響することが報告されています。`,
      `${phenomenon}は、加工方法だけでなく、素材の組合せ、接合剤、温度、圧力、試験条件によっても変化します。`,
      "静的な引張・せん断試験と、繰り返し荷重を与える疲労試験では、評価している壊れ方と寿命の意味が異なります。",
      "表面の粗さや微細構造、破断面の観察を強度データと対応させることで、効果が生じた仕組みを検討できます。",
    ],
    unknowns: [
      `${context}において、選択した加工条件が未処理条件と比べて疲労寿命をどの程度改善するかは、比較条件をそろえた検証が必要です。`,
      `${target}で得られる効果が、素材の組合せや接合条件を変えても維持されるかは十分に整理されていません。`,
      "初期強度の向上が、そのまま長期耐久性の向上につながるのか、どの破壊モードで差が生じるのかは追加の確認が必要です。",
    ],
    controversies: [
      "加工による凹凸の機械的なかみ合わせと、表面化学の変化のどちらが効果を主に説明するかは、測定方法によって解釈が分かれます。",
      "実験片で得た効果を実製品へ一般化できる範囲は、寸法、環境、荷重履歴、製造ばらつきによって変わります。",
    ],
    target_gap_deep: `先行研究では、${target}に近い異種材料接合について、表面処理や加工条件が初期の接合強度へ与える影響は個別に検討されています。一方、${context}に条件を絞り、未処理の接合面を比較対象として、${phenomenon}を繰り返し荷重下の疲労寿命と破壊モードの両面から確かめた知見は十分に整理されていません。本研究では、素材、接合剤、加工条件、試験条件を明示したうえで、処理の有無による効果量を比較し、表面・破断面の観察結果と対応させます。これにより、強度が上がるかだけでなく、どの条件で効果が維持され、どの仕組みで耐久性の差が生じるのかを明らかにします。`,
  } : isMeasurementStudy ? {
    knowns: [
  ...
  const searchQueries = isEffectStudy ? [
    `${terms.slice(0, 4).join(" ")} 先行研究`, `${target} 表面処理 接合強度`, `${target} 疲労寿命 繰り返し荷重`,
    `${phenomenon} 破壊モード`, `${context} 耐久性`, "異種材料 接合界面 レーザー加工", "CFRP 金属 接合 疲労", "laser surface texturing dissimilar material joint fatigue life",
  ] : isMeasurementStudy ? [
...
const REFERENCE_TERM_HINTS: Array<[RegExp, string[]]> = [
  [/素材|材料|金属|樹脂|CFRP|アルミ|複合部材/, ["異種材料", "接合界面", "複合材料", "材料工学"]],
  [/レーザー|表面加工|表面処理|テクスチャ/, ["レーザー加工", "表面処理", "レーザーテクスチャリング"]],
  [/強さ|強度|耐久|疲労|剥がれ|破断|繰り返し荷重/, ["接合強度", "疲労寿命", "耐久性", "破壊モード"]],
  [/銭湯|入浴|湯上がり|風呂|温泉/, ["入浴", "生活文化", "余暇", "地域"]],
  [/会話|発話|初対面|対人|人間関係|コミュニケーション/, ["コミュニケーション", "対人関係", "社会心理", "会話"]],
  [/意味|捉え|解釈|経験|印象/, ["社会心理", "生活文化", "文化"]],
  [/地域|まち|場所|公共空間/, ["地域", "生活文化", "コミュニティ"]],
  [/健康|休息|リラックス|疲れ/, ["健康", "余暇", "生活"]],
];
...
function referenceTerms(input: QuestionFreeInput, selectedRq: RQCandidate, step1: Step1Response) {
  ...
  const phrases = source.match(/[ぁ-んァ-ン一-龠々ー]{2,18}/g) || [];
  const hints = REFERENCE_TERM_HINTS.flatMap(([pattern, terms]) => pattern.test(source) ? terms : []);
  if (/R6|意味・解釈/.test(selectedRq.type_name)) hints.push("社会心理", "生活文化", "コミュニケーション");
  return unique([...hints, ...phrases.filter((value) => value.length <= 12)]).slice(0, 12);
}
...
function canonicalResourceUrl(
  ...
  if (url && /^https?:\/\//i.test(url) && item.urlType === "公式") {
...
function fallbackResources(kind: "society" | "journal", terms: string[]) {
  const rows = kind === "society" ? store.allResearchSocieties() : store.allResearchJournals();
  const matched = rows.filter((item) => terms.some((term) => `${item.name} ${item.description} ${(item.relatedFields || []).join(" ")}`.includes(term)));
...
function fallbackPaperIdeas(input: QuestionFreeInput, selectedRq: RQCandidate, step1: Step1Response) {
  ...
  const isMaterialStudy = has("異種材料") || has("材料工学") || has("接合強度") || has("レーザー加工");
  if (isMaterialStudy) {
    const material = has("レーザー加工") ? "異種材料 接合 レーザー表面加工" : "異種材料 接合界面";
    return {
      reference: [
        searchCandidate(material, "J-STAGE", "対象と加工方法に直接近い国内研究を確認する入口です。"),
        searchCandidate(`${material} 接合強度`, "CiNii Research", "表面処理と初期接合強度の関係を扱う研究を探します。"),
        searchCandidate(`${material} 疲労寿命`, "J-STAGE", "繰り返し荷重下の耐久性を扱う研究を探します。"),
      ],
      competitor: [
        searchCandidate("CFRP 金属 接合 疲労", "CiNii Research", "同じ材料系を別の接合・試験条件で扱う研究を比較します。"),
        searchCandidate("レーザーテクスチャリング 接合 破壊モード", "J-STAGE", "効果の仕組みと破壊形態を検討する競合研究を探します。"),
      ],
      adjacent: [
        searchCandidate("異種材料 接合界面 表面化学", "J-STAGE", "機械的なかみ合わせ以外の説明を補う隣接研究を探します。"),
        searchCandidate("dissimilar material joint laser surface texturing fatigue", "CiNii Research", "海外文献へ広げるための英語検索入口です。"),
      ],
    };
  }
  const topic = has("入浴") ? "銭湯 入浴" : terms.slice(0, 2).join(" ") || "日常の経験";
  const interaction = has("コミュニケーション") ? "会話 コミュニケーション" : "対人関係 社会心理";
  const context = has("生活文化") ? "生活文化 地域" : terms.slice(2, 4).join(" ") || "意味づけ";
  return {
    reference: [
      searchCandidate(topic, "J-STAGE", "テーマに直接近い先行研究を、国内学術誌から確認する入口です。"),
      searchCandidate(`${topic} 意味づけ`, "CiNii Research", "当事者が経験をどう捉えるかを扱う研究を探す入口です。"),
      searchCandidate(`${topic} 質的研究`, "J-STAGE", "インタビューや観察を用いた研究を探す入口です。"),
    ],
    competitor: [
      searchCandidate(interaction, "CiNii Research", "問いの中心となる人との関わり方について、近い研究を比べる入口です。"),
      searchCandidate(`${interaction} 初対面`, "J-STAGE", "同じ現象を別の理論や測定法で扱う研究を探す入口です。"),
    ],
    adjacent: [
      searchCandidate(context, "J-STAGE", "場面や文化の違いを扱う隣接研究を探す入口です。"),
      searchCandidate(`${context} 公共空間`, "CiNii Research", "場所・地域・生活文化の観点から読み替えるための入口です。"),
    ],
  };
}
...
function scholarlyQueries(step2: Step2Response, selectedRq: RQCandidate, terms: string[]) {
  ...
  const primary = aiQueries[0];
  if (primary) {
    const words = primary.split(/\s+/).filter(Boolean);
    return [words.slice(0, Math.max(4, words.length - 2)).join(" ")].filter((query) => query.length >= 8);
  }
...
async function fetchOpenAlexWorks(queries: string[]) {
  ...
  const ignored = new Set(["with", "from", "into", "using", "study", "research", "process", "transformation", "effect", "effects"]);
  ...
  return [...byId.values()].map((item) => {
    ...
    const titleHits = keywords.filter((word) => title.includes(word)).length;
    const abstractHits = keywords.filter((word) => abstract.includes(word)).length;
    return { ...item, relevanceScore: titleHits * 4 + abstractHits };
...
function enrichStep2References(step2: Step2Response, input: QuestionFreeInput, selectedRq: RQCandidate, step1: Step1Response) {
  const terms = referenceTerms(input, selectedRq, step1);
  const related = store.relatedResearchResources(terms, 6);
  const societies = related.societies.length ? related.societies : fallbackResources("society", terms);
  const journals = related.journals.length ? related.journals : fallbackResources("journal", terms);
  ...
  return {
    ...
    academic_mapping: {
      ...
      recommended_fields: step2.academic_mapping.recommended_fields?.length
        ? step2.academic_mapping.recommended_fields : related.fields.slice(0, 5).map((item) => asFieldResource(item, terms)),
      recommended_societies: step2.academic_mapping.recommended_societies?.length
        ? step2.academic_mapping.recommended_societies : societies.map((item) => asAcademicResource(item, "学会", terms.join("\u0000"))),
      recommended_journals: step2.academic_mapping.recommended_journals?.length
        ? step2.academic_mapping.recommended_journals : journals.map((item) => asAcademicResource(item, "ジャーナル", terms.join("\u0000"))),
    },
  };
}
