...
export default function AuthCallback() {
  ...
  return (
    <div className="auth-callback-page">
      <Helmet>
        <title>{recovery ? "パスワード再設定" : "メール確認"} ｜ MISHIRU</title>
      ...
      <section className="auth-callback-card" aria-live="polite">
        ...
        {state === "error" && <><p className="account-modal__eyebrow">メールを確認できません</p><h1>このリンクは使えません</h1><p>有効期限が切れたか、すでに使われています。アカウント画面から確認メールを送り直してください。</p><Link to="/search" className="account-secondary-button account-secondary-button--wide">MISHIRUのトップへ戻る</Link></>}
      </section>
    </div>
  );
}
