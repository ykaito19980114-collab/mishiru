...
for (const labId of ["lab-1234", "lab-7556"]) {
  const verifiedLab = labs.find((lab) => lab.id === labId);
  assert.ok(verifiedLab, `${labId}: 確認済み研究室が見つからない`);
  const evidence = assessLabEvidence(verifiedLab);
  assert.equal(evidence.canShowQuestions, true, `${labId}: 公式情報に基づく問いを表示できない`);
  assert.equal(evidence.canGenerateGuide, true, `${labId}: 確認済み研究キーワードを利用できない`);
  assert.equal(evidence.canMapResources, true, `${labId}: 関連分野を探索できない`);
  assert.equal(evidence.canSearchPapers, true, `${labId}: 所属一致の論文を確認できない`);
}
